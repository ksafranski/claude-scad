/**
 * A snapshot of the model with the marks drawn onto it.
 *
 * Scaid's studio hands one of these to the design agent, which is why it carries base64
 * alongside a preview URL. Nothing here sends it anywhere — the shape is kept so the capture
 * path is the same code in both places.
 */
export interface MarkupImage {
  ...
  /** `data:` URL, for showing it on screen. */
  previewUrl: string;
  name: string;
  /** What the picture is of, which is what tells a reader how to take it. */
  kind: "region";
}
